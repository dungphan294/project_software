...
// ## BUTTON
const int BUTTON_SERVO_PIN = 7;  // BUTTON Check pass
const int BUTTON_DOOR_PIN = 6;    // BUTTON OPEN DOOR
// ## SERVO
Servo SERVO;
...
byte byte_to_number(const int number){
    byte patterns[11] = {
            B01000000,      // 0
            B01111001,      // 1
            B00100100,      // 2
...
void save_user_input(password_t *user_input, int segments) {
    ...
    Serial.print(EEPROM_USER_MEMORY + segments);
    Serial.print(" With VALUE: ");
    Serial.println(user_input->password[segments]);
    EEPROM.update(EEPROM_USER_MEMORY + segments, user_input->password[segments]);
}
...
void loop() {
    ...
    read_password(&password);
    // USER_INPUT
    static password_t user_input = {.password ={0, 0, 0}, .attempts = 0};
    // TIME CHECK
    static unsigned long time_of_last_change = 0;
    const unsigned long current_time = millis();
    ...
    if (RE_BUTTON_state != prev_RE_BUTTON_state && (current_time - time_of_last_change > DEBOUNCE_TIME)) {
        ...
        if (RE_BUTTON_state == LOW) {
            ...
            // EEPROM Arduino Uno = 1 kilobytes
